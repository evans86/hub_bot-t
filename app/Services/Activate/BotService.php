...
class BotService extends MainService
{
    ...
    const DEFAULT_ACTIVATE_HOST = 'https://api.sms-activate.org/stubs/handler_api.php';
    ...
    {
        $bot = SmsBot::query()->where('pubc_key', $dto->public_key)->where('private_key', $dto->private_key)->first();
        if (empty($bot))
